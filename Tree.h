...
namespace tree {
	...
	protected:
		...
		bool _find(Node<T>* subroot, const Key& key, T& item) const {
			if (subroot == NULL) {
				return false;
	...
	public:
		...
		void print() const {
			if (root == NULL) {
				cout << "The k-d tree is empty." << endl;
			} else {
				_print(root, 0);
			}
		}
	...
	template <typename T>
	class KDTree : public BSTree<T, int*>
	{
	private:
		size_t ndim;
		bool _find(Node<T>* subroot, int* coord, T& item, int discrim=0) const {
			if (subroot == NULL) return false;  // emtpy tree
		...
		Node<T>* _insert(Node<T>* subroot, const T& item, int discrim=0) {
			if (subroot == NULL) {
				// empty tree: create node
				return new Node<T>(item);
			}
	...
	public:
		KDTree(size_t nDimensions) : BSTree<T, int*>(), ndim(nDimensions) {}
		~KDTree() {}
	};
