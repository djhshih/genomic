...
BlockScanResult tmax_core(const std::vector<double>& data, double tss, int al0, bool ibin, bool track_location) {
    ...
    std::vector<double> bpsmax(nb + 1), bpsmin(nb + 1), bssbij(nb2 + 1), bssijmax(nb2 + 1);
    std::vector<int> bb(nb + 1), ibmin(nb + 1), ibmax(nb + 1), bloci(nb2 + 1), blocj(nb2 + 1), loc(nb2 + 1), alen(nb2 + 1);

    for (int i = 1; i <= nb; ++i) {
        bb[i] = static_cast<int>(std::lround(rn * (static_cast<double>(i) / static_cast<double>(nb))));
    }
    ...
    for (int j = 1; j <= nb; ++j) {
        sx[ilo] = psum + data[ilo - 1];
        double psmin = sx[ilo];
        int ipsmin = ilo;
        double psmax = sx[ilo];
        int ipsmax = ilo;
        for (int i = ilo + 1; i <= bb[j]; ++i) {
            sx[i] = sx[i - 1] + data[i - 1];
            if (sx[i] < psmin) {
                psmin = sx[i];
                ipsmin = i;
            }
    ...
    double bssmax = 0.0;
    int tmaxi = std::min(ipsmax0, ipsmin0);
    int tmaxj = std::max(ipsmax0, ipsmin0);
    if (psdiff > 0.0) {
    ...
    if (psdiff > 0.0) {
        ...
        for (int i = 1; i <= nb; ++i) {
            for (int j = i; j <= nb; ++j) {
                ...
                const double denom = std::min(rjlo * (rn - rjlo), rjhi * (rn - rjhi));
                const double rnjov1 = rn / denom;
        ...
        const int nb1 = l;
        std::sort(loc.begin() + 1, loc.begin() + nb1 + 1, [&](int a, int b) { return bssbij[a] < bssbij[b]; });

        for (int ll = nb1; ll >= 1; --ll) {
            const int k = loc[ll];
            if (bssmax > bssijmax[k]) continue;
            const int bi = bloci[k];
            const int bj = blocj[k];
            int alenmax = alen[k];
    ...
    return {bssmax, track_location ? tmaxi : 0, track_location ? tmaxj : 0};
}
...
double htmaxp(const std::vector<double>& px, double tss, int k, int al0, bool ibin) {
    ...
    auto eval_range = [&](int ilo1, int ihi, double psdiff) {
        const double psdiffsq = ibin ? std::pow(psdiff - 0.5, 2.0) : psdiff * psdiff;
        for (int j = al0; j <= k; ++j) {
            ...
            for (int i = ilo1; i <= ihi - j; ++i) {
                sxmx = std::max(sxmx, std::abs(sx[i + j] - sx[i]));
            }
            const double bssmx = ibin ? fac * std::pow(sxmx - 0.5, 2.0) : fac * sxmx * sxmx;
            out = std::max(out, bssmx);
        }
    };

    eval_range(1, bb[1], bpsmax[1] - bpsmin[1]);

    {
        ...
        for (int j = al0; j <= k; ++j) {
            ...
            const int nmj = n - j;
            for (int i = 1; i <= j; ++i) {
                sxmx = std::max(sxmx, std::abs(sx[i + nmj] - sx[i]));
            }
            const double bssmx = ibin ? fac * std::pow(sxmx - 0.5, 2.0) : fac * sxmx * sxmx;
            out = std::max(out, bssmx);
        }
    ...
    for (int l = 2; l <= nb; ++l) {
        eval_range(bb[l - 1] + 1, bb[l], bpsmax[l] - bpsmin[l]);
...
double tpermp(int n1, int n2, int n, const double* x, std::vector<double>& px, int nperm, std::mt19937_64& rng) {
    ...
    if (n1 <= n2) {
        m1 = n1;
        rm1 = rn1;
        ostat = 0.99999 * std::abs(xsum1 / rn1 - xbar);
        tstat = (ostat * ostat) * rn1 * rn / rn2;
    } else {
        m1 = n2;
        rm1 = rn2;
        ostat = 0.99999 * std::abs(xsum2 / rn2 - xbar);
        tstat = (ostat * ostat) * rn2 * rn / rn1;
    }
...
void getmncwt(int n, const std::vector<double>& cwts, int k, std::vector<double>& mncwt, double& delta) {
    ...
    const double rn = cwts.at(n - 1);
    for (int j = 1; j <= k; ++j) {
        mncwt[j] = cwts[j - 1];
        const int nmj = n - j;
        for (int i = 1; i <= nmj; ++i) {
            const double rj = cwts[i + j - 1] - cwts[i - 1];
            mncwt[j] = std::min(mncwt[j], rj);
        }
        for (int i = 1; i <= j; ++i) {
            const double rj = cwts[i + nmj - 1] - cwts[i - 1];
            mncwt[j] = std::min(mncwt[j], rn - rj);
        }
    }
    const int j = k + 1;
    const int nmj = n - j;
    ...
    for (int i = 1; i <= nmj; ++i) {
        const double rj = cwts[i + j - 1] - cwts[i - 1];
        delta = std::min(delta, rj);
    }
    for (int i = 1; i <= j; ++i) {
        const double rj = cwts[i + nmj - 1] - cwts[i - 1];
        delta = std::min(delta, rn - rj);
    }
    delta /= cwts.at(n - 1);
}
...
BinarySegmentationResult wtmaxo(const std::vector<double>& x,
    ...
    if (psdiff > 0.0) {
        ...
        const double psrnj = psrj * (psrn - psrj);
        bssmax = (psdiff * psdiff) / psrnj;
        tmaxi = std::min(ipsmax0, ipsmin0);
        tmaxj = std::max(ipsmax0, ipsmin0);
        ...
        for (int i = 1; i <= nb; ++i) {
            for (int j = i; j <= nb; ++j) {
                ...
                if (jhi - ilo1 > nal0) {
                    awthi = 0.0;
                    for (int kk = 1; kk <= al0; ++kk) {
                        awthi = std::max(awthi, cwts[nal0 + kk - 1] - cwts[kk - 1]);
                    }
                }
                double awtlo;
                if (i == j) {
                    awtlo = cwts[ilo1 + al0 - 1] - cwts[ilo1 - 1];
                    for (int kk = ilo1 + 1; kk <= ihi - al0; ++kk) {
                        awtlo = std::min(awtlo, cwts[kk + al0 - 1] - cwts[kk - 1]);
                    }
                } else if (i + 1 == j) {
                    awtlo = cwts[jlo - 1] - cwts[jlo - al0 - 1];
                    for (int kk = jlo - al0 + 1; kk <= ihi; ++kk) {
                        awtlo = std::min(awtlo, cwts[kk + al0 - 1] - cwts[kk - 1]);
                    }
                } else {
                ...
                const double sij2 = std::abs(bpsmax[i] - bpsmin[j]);
                const double sijmx0 = std::max(sij1, sij2);
                const double psrnj = std::min(awtlo * (psrn - awtlo), awthi * (psrn - awthi));
                const double bsslim = (sijmx0 * sijmx0) / psrnj;
                if (bssmax <= bsslim) {
                    ++l;
                    loc[l] = l;
                    bloci[l] = i;
...
double wtmaxp(const std::vector<double>& px,
    ...
    int ilo = 1;
    double psum = 0.0, psmin0 = 0.0, psmax0 = 0.0, ssq = 0.0;
    int ipsmin0 = n, ipsmax0 = n;
    for (int j = 1; j <= nb; ++j) {
        sx[ilo] = psum + px[ilo - 1] * wts[ilo - 1];
        ssq += px[ilo - 1] * px[ilo - 1] * wts[ilo - 1];
        double psmin = sx[ilo], psmax = sx[ilo];
        int ipsmin = ilo, ipsmax = ilo;
        for (int i = ilo + 1; i <= bb[j]; ++i) {
            sx[i] = sx[i - 1] + px[i - 1] * wts[i - 1];
            ssq += px[i - 1] * px[i - 1] * wts[i - 1];
            if (sx[i] < psmin) {
                psmin = sx[i];
                ipsmin = i;
            }
    ...
    const double psdiff = psmax0 - psmin0;
    const double psrn = cwts[n - 1];
    double tss = ssq - std::pow(sx[n] / psrn, 2.0);
    const double psrj = std::abs(cwts[ipsmax0 - 1] - cwts[ipsmin0 - 1]);
    double bssmax = (psdiff * psdiff) / (psrj * (psrn - psrj));
    const double psrnov2 = psrn / 2.0;
    ...
    if (tss <= bssmax + 0.0001) tss = bssmax + 1.0;
    return bssmax / ((tss - bssmax) / (rn - 2.0));
}
...
double hwtmaxp(const std::vector<double>& px,
    ...
    for (int j = 1; j <= nb; ++j) {
        ...
        if (d <= k && d >= al0) {
            ...
            const double rnj = rj * (rn - rj);
            const double bssij = std::pow(bpsmax[j] - bpsmin[j], 2.0) / rnj;
            if (bssmax < bssij) bssmax = bssij;
        }
    }
    double tss = ssq - std::pow(sx[n] / rn, 2.0);

    auto eval_range = [&](int left, int right, double psdiff) {
...
ChangePointResult fndcpt(const std::vector<double>& x,
    ...
    int l = std::min(obs.end - obs.start, n - obs.end + obs.start);
    if ((ostat1 >= 7.0) && (l >= 10)) {
    } else {
        int nrej = 0;
        if (hybrid) {
            ...
            const double pval2 = cpval - pval1;
            const int nrejc = static_cast<int>(pval2 * static_cast<double>(nperm));
